...
function ScanQR() {
        ...
        try {
          ...
          if (data.type === 'activity-checkin' && data.atividadeId) {
            ...
            setCheckinAtividade(
              ativ || {
                id: data.atividadeId,
                nome: 'Atividade desconhecida',
              }
            );
          } else if (data.presencaId) {
            fetch(`http://localhost:3001/presencas/${data.presencaId}`)
              .then((response) => {
                if (!response.ok) {
                  throw new Error('Presença não encontrada no sistema.');
                }
  ...
  }, [scannerAtivo, atividades]);

  const confirmarPresenca = () => {
    if (!resultado) return;

    fetch(`http://localhost:3001/presencas/${resultado.id}`, {
      method: 'PATCH',
      headers: {
      ...
      .then((presencaAtualizada) => {
        setResultado(presencaAtualizada);
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  const handleCheckin = (e) => {
    ...
    fetch(
      `http://localhost:3001/presencas?nome=${checkinNome.trim()}&atividadeId=${checkinAtividade.id}`
    )
      .then((response) => {
        if (!response.ok) {
      ...
      .then((presencasEncontradas) => {
        const duplicado = presencasEncontradas.find(
          (p) =>
            p.nome.toLowerCase() === checkinNome.trim().toLowerCase() &&
            p.atividadeId === checkinAtividade.id
        );

        if (duplicado) {
          setCheckinFeito(duplicado);
          return;
        }

        const novaPresenca = {
          nome: checkinNome.trim(),
          matricula: '',
          atividadeId: checkinAtividade.id,
          local: checkinAtividade.local || '',
          verificado: true,
          verificadoEm: new Date().toISOString(),
        ...
        fetch('http://localhost:3001/presencas', {
          ...
          .then((presencaCriada) => {
            setCheckinFeito(presencaCriada);
          });
      })
      .catch((error) => {
        console.error(error);
      ...
      <div className="content" style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        >
          <div style={{ fontSize: 15, fontWeight: 800 }}>
            {checkinAtividade.nome}
          </div>

          {checkinAtividade.data && (
            <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>
              {checkinAtividade.data} · {checkinAtividade.horas}h ·{' '}
              {checkinAtividade.local}
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleCheckin}>
          <div className="form-group">
            ...
            <input
              ...
              value={checkinNome}
              onChange={(e) => setCheckinNome(e.target.value)}
              required
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 6 }}>
            Registrado em{' '}
            {new Date(
              checkinFeito.verificadoEm || checkinFeito.timestamp
            ...
            {atividade && (
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 24, marginTop: 10 }}>
                  <div className="meta-item">
                    Data<strong>{atividade.data}</strong>
                  </div>

                  <div className="meta-item">
                    Horas<strong>{atividade.horas}h</strong>
                  </div>
            ...
            {resultado.verificado ? (
              ...
              >
                ...
                >
                  ...
                  {new Date(resultado.verificadoEm).toLocaleString('pt-BR')}
                </div>
              </div>
            ) : (
              <div>
  ...
  return (
    <div className="content" style={{ maxWidth: 520, margin: '0 auto' }}>
      ...
      <p className="page-sub">
        Escaneie o QR code do aluno para verificar presença, ou o QR code da
        atividade para fazer check-in direto.
      </p>
